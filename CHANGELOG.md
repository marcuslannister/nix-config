...
- Document a one-time manual prerequisite: `brew trust --tap d12frosted/emacs-plus`, run both normally and under `env -u XDG_CONFIG_HOME`. Homebrew refuses formulae from untrusted third-party taps, nix-darwin has no option for this, and because it invokes `sudo --user=... --set-home brew bundle` and sudo strips `XDG_CONFIG_HOME`, brew reads the `~/.homebrew/trust.json` fallback rather than `~/.config/homebrew/trust.json`. Brew owns both files, so neither can be managed by home-manager.
