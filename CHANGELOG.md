...
- Deploy `~/.homebrew/trust.json` as a user-owned out-of-store symlink before Homebrew Bundle runs, so `d12frosted/emacs-plus` is trusted without a manual command and later `brew trust` writes update the dotfiles repository.
